...
> **版本說明**：本節示例基於 Docker v29.x 和常見映像檔版本編寫。示例中的版本號（如 `nginx:1.28`、`mysql:8.4`、`mysql:5.7` 等）為演示用途。實際使用時請訪問 [Docker Hub 官方頁面](https://hub.docker.com) 或相應映像檔的釋出頁確認最新可用版本和標籤。
...
```
詳細配置方法請參考[映像檔加速器](../03_install/3.9_mirror.md)章節。
...
Docker Hub 的[官方映像檔](https://hub.docker.com/search?q=&type=image&image_filter=official) (標有 “Official Image” 標識) 經過 Docker 團隊稽核，相對更安全。示例如下：
