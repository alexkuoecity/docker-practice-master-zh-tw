...
- `docker compose ps`：檢視專案中的容器狀態
- `docker compose logs`：檢視容器日誌（排查問題的第一步）
- `docker compose exec`：進入正在執行的容器執行命令

**構建與更新**：

- `docker compose build`：重新構建映像檔（修改 Dockerfile 後）
...
* `--verbose` 輸出更多除錯資訊。(**已棄用**：在 Docker Compose V2 中，請改用 `docker --log-level debug compose ...` 或設定環境變數 `COMPOSE_DEBUG=1`。)
...
* `--no-cache` 構建映像檔過程中不使用 cache (這將加長構建過程)。

* `--pull` 始終嘗試透過 pull 來獲取更新版本的映像檔。
...
* `--protocol=proto` 指定埠協議，tcp (預設值) 或者 udp。
...
> **說明**：有些舊環境或實驗性入口中仍可能出現 `docker compose scale`，但它不應再被視為當前 Compose 的通用穩定預設命令。
...
* `-d` 後臺啟動。
* `--scale SERVICE=NUM` 指定服務例項數量，可重複使用。
