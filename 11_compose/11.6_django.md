...
- `web` 服務執行 Django 應用，對外暴露 8000 埠
...
| `COPY requirements.txt` 在前 | 分層複製 | 只有 requirements.txt 變化時才重新安裝依賴，加速構建 |
...
db:
  ...
  healthcheck:                          # 健康檢查，確保資料庫就緒
    ...
    interval: 5s
```
> ⚠️ **筆者提醒**：`volumes` 配置很重要！沒有它，每次容器重啟資料都會丟失。筆者見過不少新手因為忘記這一步，導致開發資料全部丟失。
...
| 配置項 | 作用 | 筆者建議 |
|--------|------|---------|
| `volumes: .:/code` | 程式碼掛載 | 開發時必備，修改程式碼無需重新構建映像檔 |
| `depends_on` + `healthcheck` | 啟動順序 | 確保資料庫就緒後 Django 才啟動，避免連線錯誤 |
| `environment` | 環境變數 | 推薦用環境變數管理配置，避免硬編碼 |
...
2. **Volume 掛載問題**：檢查 `compose.yaml` 中的 volumes 配置
...
- [Compose 模板檔案詳解](11.5_compose_file.md)：深入理解 Compose 檔案的所有配置項
- [使用 WordPress](11.8_wordpress.md)：另一個 Compose 實戰案例
- [Dockerfile 最佳實踐](../appendix/best_practices.md)：構建更小、更安全的映像檔
- [資料管理](../08_data/README.md)：Volume 和資料持久化詳解
