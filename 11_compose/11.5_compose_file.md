...
`deploy` 用於描述副本數、更新策略、資源限制等部署引數。該欄位在 Swarm 中支援最完整；在本地 `docker compose up` 場景下通常只有部分欄位生效。
...
|------|------|
| `no` | 預設值，不自動重啟 |
| `always` | 無論退出碼如何，始終重啟；Docker 守護程序啟動時也會重啟 |
| `on-failure[:max-retries]` | 僅在非零退出碼時重啟，可指定最大重試次數 |
| `unless-stopped` | 類似 `always`，但手動停止的容器在守護程序重啟後不會自動啟動 |
