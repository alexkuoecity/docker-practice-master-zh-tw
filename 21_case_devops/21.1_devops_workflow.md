...
2. **Build**：GitLab CI 觸發構建任務。
3. **Test**：執行單元測試和整合測試。
4. **Package**：構建 Docker 映像檔並推送到 Harbor/Registry。
...
1. **不可變基礎設施**：一旦映像檔構建完成，在各個環境（Dev、Staging、Prod）中都應該使用同一個映像檔 tag（通常是 commit hash），而不是重新構建。
2. **配置分離**：使用 ConfigMap 和 Secret 管理環境特定的配置，不要打包進映像檔。
3. **應對 Docker Hub 限額 (Rate Limits)**：
   - Docker Hub 對匿名拉取實施了嚴格的限制 (6 小時內約 100 次)。若在 CI/CD 中頻繁構建，極易觸發 `toomanyrequests` 錯誤。
   - **最佳策略**：
...
4. **GitOps**：考慮引入 ArgoCD，將部署配置也作為程式碼儲存在 Git 中，實現 Git 驅動的部署同步。
