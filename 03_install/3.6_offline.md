...
> 注意：Docker 官方當前支援的 RHEL 相容平臺基線已是 **CentOS Stream 9/10**（具體以官方 [安裝文件](https://docs.docker.com/engine/install/centos/) 為準）。下面的離線示例建議統一按 `el9` 軟體包和 `dnf` 流程準備，Rocky Linux 9、AlmaLinux 9 也可先在測試環境按相同思路驗證。
