...
> 在一些基於 Debian 的 Linux 發行版中，`$(. /etc/os-release && echo "$VERSION_CODENAME")` 可能不會返回 Debian 官方倉庫使用的代號，例如 [Kali Linux](https://www.kali.org/docs/policy/kali-linux-relationship-with-debian/) 等衍生發行版。此時請手動替換為對應的 Debian 代號，例如 `bookworm`。
