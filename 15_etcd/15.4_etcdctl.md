...
> **版本說明：** 本節示例基於 etcd 3.5 系列版本。請訪問 [etcd 官方釋出頁](https://github.com/etcd-io/etcd/releases) 獲取最新版本資訊。

`etcdctl` 是一個命令列客戶端，它能提供一些簡潔的命令，供使用者直接跟 `etcd` 服務打交道，而無需基於 `HTTP API` 方式。這在某些情況下將很方便，例如使用者對服務進行測試或者手動修改資料庫內容。我們也推薦在剛接觸 `etcd` 時透過 `etcdctl` 命令來熟悉相關的操作，這些操作跟 `HTTP API` 實際上是對應的。

`etcd` 專案二進位制發行包中已經包含了 `etcdctl` 工具，沒有的話，可以從 [github.com/etcd-io/etcd/releases](https://github.com/etcd-io/etcd/releases) 下載。
