...
- **nerdctl**：這是一個由 containerd 專案維護者開發的，完全相容 Docker CLI 體驗的 containerd 命令列用戶端。對於習慣了 `docker run/ps/build` 命令的使用者來說，`nerdctl` 可以作為直接操作 containerd 的理想替代品，並且它還支援直接構建映像檔（依賴 BuildKit）。
