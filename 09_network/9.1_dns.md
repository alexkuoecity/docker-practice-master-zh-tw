...
1. **預設 Bridge 網路**：繼承宿主機的 DNS 配置 (`/etc/resolv.conf`)。
...
1. 宿主機的 `/etc/resolv.conf` 可能有問題 (例如使用了本地迴環地址 127.0.0.53，特別是 Ubuntu 系統)。Docker 可能會嘗試修復，但有時會失敗。
2. 嘗試手動指定 DNS：`docker run --dns 8.8.8.8 ...`
3. 檢查防火牆是否攔截了 UDP 53 埠。
