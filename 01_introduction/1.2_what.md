...
> `runc` 是一個 Linux 命令列工具，用於根據 [OCI 容器執行時規範](https://github.com/opencontainers/runtime-spec)建立和執行容器。

> `containerd` 是一個守護程式，它管理容器生命週期，提供了在一個節點上執行容器和管理映像檔的最小功能集。

### 1.2.5 Docker 的歷史與生態

**Docker** 最初是 `dotCloud` 公司創始人 [Solomon Hykes](https://github.com/shykes) 在法國期間發起的一個公司內部專案，於 [2013 年 3 月以 Apache 2.0 授權協議開源](https://en.wikipedia.org/wiki/Docker_%28software%29)。
