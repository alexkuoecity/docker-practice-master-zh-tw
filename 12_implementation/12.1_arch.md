...
Docker 採用了 **C/S (客戶端/服務端)** 架構。Client 向 Daemon 傳送請求，Daemon 負責構建、執行和分發容器。

```mermaid
graph LR
  C1["客戶端"] -->|docker run| D["dockerd<br/>守護程序"]
  C1 -->|docker pull| D
...
- [名稱空間](./12.2_namespace.md)：Runc 如何隔離容器
- [控制組](./12.3_cgroups.md)：Runc 如何限制資源
- [聯合檔案系統](./12.4_ufs.md)：映像檔如何儲存
