...
`docker buildx` 是構建多架構映像檔的最佳實踐工具，它遮蔽了底層的複雜性，提供了一鍵構建多架構映像檔的能力。
...
*   `TARGETPLATFORM`：構建映像檔的目標平臺，例如 `linux/amd64`。
*   `TARGETOS`：目標平臺的作業系統，例如 `linux`。
*   `TARGETARCH`：目標平臺的架構，例如 `amd64`。
*   `TARGETVARIANT`：目標平臺的變種，例如 `v7`。
*   `BUILDPLATFORM`：構建環境的平臺。
*   `BUILDOS`：構建環境的作業系統。
*   `BUILDARCH`：構建環境的架構。
