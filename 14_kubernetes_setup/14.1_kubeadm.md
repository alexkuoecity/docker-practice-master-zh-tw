...
參考[安裝 Docker](../03_install/README.md) 一節新增 apt/yum 源，之後執行如下命令。
...
> **提示**：`containerd config default` 生成的預設配置會自動使用當前版本的正確路徑。如果你使用的是 containerd 2.x，配置中的外掛路徑將以 `io.containerd.cri.v1.runtime` 開頭；如果仍在使用 1.x，則為 `io.containerd.grpc.v1.cri`。
