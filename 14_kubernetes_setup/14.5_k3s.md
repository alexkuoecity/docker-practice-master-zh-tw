...
[K3s](https://k3s.io/) 是一個輕量級的 Kubernetes 發行版，由 Rancher Labs 開發。它專為邊緣計算、物聯網、CI、ARM 等資源受限的環境設計。K3s 被打包為單個二進位制檔案，只有不到 100MB，但透過了 CNCF 的一致性測試。
...
> **版本說明**：輸出中的 `v1.36.0+k3s1` 為編寫本文件時的版本示例。實際輸出版本號取決於你所安裝的 K3s 版本。更多資訊請參見 [K3s Releases](https://github.com/k3s-io/k3s/releases)。
